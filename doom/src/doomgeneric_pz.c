...
//
// Video: writes raw RGBA frames (320x200x4) to stdout
...
//
// Protocol:
//   stdout: [RGBA pixels, 256000 bytes per frame, continuous]
...
static void readStdinKeys(void)
{
    ...
    while (1) {
        ...
        if (n == 2) {
            ...
            // EAGAIN = no data available, or pipe closed
            break;
        } else {
            // Got 1 byte — read the second byte (blocking briefly)
            ssize_t n2 = read(STDIN_FILENO, buf + 1, 1);
            if (n2 == 1) {
                addKeyToQueue(buf[0], buf[1]);
            } else {
                break;
...
{
#ifdef _WIN32
    // Set stdout to binary mode (prevent \n → \r\n conversion)
    ...
    // Try to increase stdout pipe buffer for smoother frame delivery
    // F_SETPIPE_SZ may not be available on all systems
#ifdef F_SETPIPE_SZ
    fcntl(STDOUT_FILENO, F_SETPIPE_SZ, 1048576); // 1MB, ignore failure
#endif
...
void DG_DrawFrame(void)
{
    ...
    size_t written = fwrite(s_RgbaBuf, 1, sizeof(s_RgbaBuf), stdout);
